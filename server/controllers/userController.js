...
export const updateUserData = async (req, res) => {
    try {
        ...
        if (tempUser.username !== username) {
            ...
            if (user) {
                //we will not allow to change username
...
export const unfollowUser = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId);
        user.following = user.following.filter(user => user.toString() !== id);  //change added toString()
        await user.save();
...
export const sendConnectionRequest = async(req,res)=>{
    try {
        ...
        const connectionRequests=await Connection.find({
            ...
            //Check if connection request already connectd
        ...
        console.log(error);
        res.json({success:false,message:error.messagee});    
    }
}
...
export const getUserConnections = async(req,res)=>{
    try {
        ...
        const pendingConnections = (await Connection.find({to_user_id:userId,
            status:'pending'}).pupulate('from_user_id')).map(connection=>connection.from_user_id)
