    ...
    <div className='bg-white rounded-xl shadow p-4 space-y-4 w-full max-w-2xl'>
        ...
        <div onClick={()=>navigate(`/profile/`+post.user._id)} className='inline-flex items-center gap-3 cursor-pointer'>
            ...
            <div>
                ...
                <div className='text-gray-500 text-sm'>@{post.user.username}.{moment(post.creatdAt).fromNow()}</div>
            </div>
        ...
        {/* Post content */}
        {
            ...
            whitespace-pre-line' dangerouslySetInnerHTML={{__html: postWithHashtags}}></div>
        }
        {/* Images */}
        <div className='grid grid-cols-2 gap-2'>
            {post.image_urls.map((img,index)=>(
                <img src={img} key={index} alt="" className={`w-full h-48 object-cover
        ...
        border-gray-300'>
            <div className='flex items-center gap-1'>
                <Heart className={`w-4 h-4 cursor-pointer ${likes.includes
                    (currentUser.id) && 'text-red-500 fill-red-500'}`} onClick={handleLike}/>
