...
const RecentMessages = () => {
    ...
    const fetchRecentMessagges = async () => {
        try {
    ...
    useEffect(() => {
        if(user){
            fetchRecentMessagges()
            setInterval(fetchRecentMessagges,30000)
            ...
            <div className='flex flex-col max-h-56 overflow-y-scroll no-scrollbar'>
                {
                    messages.map((message, index) => (
                        <Link to={`/messages/${message.from_user_id._id}`} key={index} className='flex items-start gap-2 py-2
                    hover:bg-slate-100'>
                            ...
                            <div className='w-full'>
                                ...
                                <div className='flex justify-between'>
                                    ...
                                    {!message.seen && 
                                    <p className='bg-indigo-500 text-white w-4 h-4 flex items-center justify-center
