      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='flex flex-wrap gap-6 mt-6'>
          {dataArray.find((item)=>item.label===currentTab).value.map((user)=>(
            <div key={user.id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
              ...
              <div className='flex-1'>
                ...
                <div className='flex max-sm:flex-col gap-2 mt-4'>
                  {
                    <button onClick={()=>navigate(`/profile/${user._id}`)} className='w-full p-2 text-sm rounded bg-linear-to-r
                    from-indigo-500 to-purple-600 hover:from-indigo-600
                    hover:to-purple-700 active:scale-95 transition text-white
                    cursor-pointer'>
                      view Profile
                    </button>
                  }
                  ...
                  }
                  {
                    currentTab === 'Connections' && (
                      <button onClick={()=>navigate(`/messages/${user._id}`)} className='s-full p-2 text-sm rounded bg-slate-100
